...
import type { Todo, Heading, PushRequest } from './types.js';
...
export function isValidTodo(obj: unknown): obj is Partial<Todo> {
  ...
  if (todo.status !== undefined && !['open', 'completed', 'canceled'].includes(todo.status as string)) return false;
...
export function isValidHeading(obj: unknown): obj is Partial<Heading> {
  ...
  if (heading.title !== undefined && typeof heading.title !== 'string') return false;
  if (heading.position !== undefined && typeof heading.position !== 'number') return false;
